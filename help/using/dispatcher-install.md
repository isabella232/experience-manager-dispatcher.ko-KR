...
운영 체제 및 웹 서버에 대한 최신 Dispatcher 설치 파일을 얻으려면 [Dispatcher 릴리스 노트](release-notes.md) 페이지를 사용합니다. 디스패처 릴리스 번호는 Adobe Experience Manager 릴리스 번호와 독립적으로 제공되며 Adobe Experience Manager 6.x, 5.x 및 Adobe CQ 5.x 릴리스와 호환됩니다.
...
| 로그 파일 | `dispatcher.log` 파일의 위치입니다. 이 설정이 안 되면 로그 메시지가 windows 이벤트 로그로 이동합니다. |
...
| 재허가 | HTTP 요청의 인증 헤더를 처리하는 방법을 지정합니다. 다음 값이 유효합니다.<br/>0 - 인증 헤더가 수정되지 않습니다. <br/>1 - &quot;Basic&quot;이 아닌 &quot;Authorization&quot;이라는 헤더를  `Basic <IIS:LOGON\_USER>` 상응하는 머리글로 바꿉니다.<br/> |
...
1. [처리기 매핑] 페이지의 [작업] 패널에서 [와일드카드 스크립트 맵 추가]를 클릭하고 다음 속성 값을 추가한 다음 [확인]을 클릭합니다.
   ...
   * 이름:처리기 매핑의 설명형 이름(예: `Dispatcher`).
...
1. 발송자 캐시로 사용 중인 웹 사이트의 루트 폴더를 마우스 오른쪽 단추로 클릭하고 속성(예: `C:\inetpub\wwwroot`)을 클릭합니다.
...
1. 디스패처 폴더의 권한 대화 상자에서 방금 추가한 계정을 선택하고 전체 제어&#x200B;**을 제외한 계정**&#x200B;에 대한 모든 권한을 활성화하고 확인을 클릭합니다. 확인을 클릭하여 폴더 속성 대화 상자를 닫습니다.
...
1. 이름 확인 단추를 클릭합니다. Windows에서 사용자 계정을 확인하면 확인을 클릭합니다.
1. 발송자 폴더의 권한 대화 상자에서 방금 추가한 계정을 선택하고 전체 제어,**을 제외한 계정**&#x200B;에 대한 모든 권한을 활성화한 다음 확인을 클릭합니다. 확인을 클릭하여 폴더 속성 대화 상자를 닫습니다.
...
* [Dispatcher](page-invalidate.md) 와 함께 작동하도록 AEM을 구성합니다.

## Apache 웹 서버 {#apache-web-server}

>[!CAUTION]
>
>**Windows** 및 **Unix** 모두 아래 설치 지침이 여기에 나와 있습니다. 단계 수행 시 주의하십시오.

### Apache 웹 서버 설치 {#installing-apache-web-server}

Apache 웹 서버 설치 방법에 대한 자세한 내용은 설치 설명서를 참조하십시오. [online](https://httpd.apache.org/) 또는 배포 중 하나를 참조하십시오.

>[!CAUTION]
>
>소스 파일을 컴파일하여 Apache 바이너리를 만드는 경우 **동적 모듈 지원**&#x200B;을(를) 켜야 합니다. 이 작업은 **—enable-shared** 옵션 중 하나를 사용하여 수행할 수 있습니다. 최소 구성에는 `mod_so` 모듈이 포함됩니다.
>
...
1. Dispatcher 파일을 해당 Apache 모듈 디렉토리에 배치합니다.
   ...
   * **Unix**:설치에 따라  `<APACHE_ROOT>/libexec` 또는  `<APACHE_ROOT>/modules`디렉토리를 찾습니다.\
      ...
      장기 유지 관리를 단순화하기 위해 디스패처에 대해 `mod_dispatcher.so`이라는 상징적 링크를 만들 수도 있습니다.\
      `ln -s dispatcher-apache<x>-<os>-<rel-nr>.so mod_dispatcher.so`

1. dispatcher.any 파일을 `<APACHE_ROOT>/conf` 디렉토리로 복사합니다.

   **참고:** Dispatcher 모듈의 DispatcherLog 속성이 그에 따라 구성되어 있으면 이 파일을 다른 위치에 배치할 수 있습니다. (아래 발송자별 구성 항목을 참조하십시오.)
...
터미널 창에 다음 명령을 입력하고 `[path to the dispatcher.so file]`을 Apache Web Server에 설치한 Dispatcher 모듈의 경로로 대체하고, *`path to the docroot`*&#x200B;은(는) docroot가 위치한 경로로 바꿉니다(예:).`/opt/cq/cache`):
...
`httpd.conf`을(를) 사용하여 Apache 웹 서버를 구성해야 합니다. Dispatcher 설치 키트에서 `httpd.conf.disp<x>`이라는 이름의 예제 구성 파일을 보게 됩니다.
...
1. 나열된 순서대로 다음 구성 항목을 추가해야 합니다.
   ...
   * **Load** Mode를 사용하여 시작 시 모듈을 로드합니다.
   * **DispatcherConfig, DispatcherLog** 및 **DispatcherLogLevel**&#x200B;을 비롯한 발송자별 구성 항목입니다.
   * **Set** Handler를 활성화하여 발송자를 활성화합니다. **LoadModule**.
   * **** ModMimeUsePathInfoto configure behavior **_mime**.
...
| DispatcherLogLevel | 로그 파일의 로그 수준:<br/>0 - 오류 <br/>1 - 경고 <br/>2 - 정보 <br/>3 - 디버그 <br/>**참고**:설치 및 테스트 중에 로그 수준을 3으로 설정한 다음 프로덕션 환경에서 실행할 때 0으로 설정하는 것이 좋습니다. |
...
| DispatcherDeclineRoot | 루트 &quot;/&quot;에 대한 요청을 거절할지 여부를 정의합니다.<br/>**0** - / <br/>**1**&#x200B;에 대한 요청을 수락합니다. / 요청이 디스패처에 의해 처리되지 않습니다.올바른 매핑에 mod_alias를 사용하십시오. |
...
| DispatcherPassError | ErrorDocument 처리에 대한 오류 코드를 지원하는 방법을 정의합니다.<br/>**0** - Dispatcher는 클라이언트에 대한 모든 오류 응답을 스풀합니다. <br/>**1** - 디스패처는 클라이언트에 오류 응답을 스풀하지 않고(상태 코드가 400보다 크거나 같은 경우) 상태 코드를 Apache로 전달하며, 이 상태 코드는 ErrorDocument 지시어가 이러한 상태 코드를 처리할 수 있도록 합니다. <br/>**코드 범위**  - Apache로 응답이 전달되는 오류 코드 범위를 지정합니다. 다른 오류 코드가 클라이언트에 전달됩니다. 예를 들어 다음 구성은 오류 412에 대한 응답을 클라이언트에 전달하며 다른 모든 오류가 Apache로 전달됩니다.DispatcherPassError 400-411,413-417 |
...
ServerTokens Prod`\
자세한 내용은 [Apache Documentation about ServerTokens Directive(예: Apache 2.4)](https://httpd.apache.org/docs/2.4/mod/core.html)을 참조하십시오.

**SetHandler**

이러한 항목 후에는 Dispatcher가 들어오는 요청을 처리하기 위해 구성 컨텍스트( `<Directory>`, `<Location>`)에 **SetHandler** 문을 추가해야 합니다. 다음 예제에서는 Dispatcher가 전체 웹 사이트에 대한 요청을 처리하도록 구성합니다.
...
>[!NOTE]
이 이름은 모듈에 정의된 핸들러의 이름이므로 **SetHandler** 문의 매개 변수는 위 예&#x200B;*에서와 정확히 동일하게 기록되어야 합니다.*
이 명령에 대한 자세한 내용은 제공된 예제 구성 파일 및 Apache Web Server 설명서를 참조하십시오.
...
mod_mime 모듈(예: [Apache Module mod_mime](https://httpd.apache.org/docs/2.4/mod/mod_mime.html))은 HTTP 응답으로 선택한 컨텐츠에 컨텐츠 메타데이터를 할당하는 데 사용됩니다. 기본 설정은 mod_mime이 컨텐츠 유형을 결정하는 경우 파일이나 디렉토리에 매핑되는 URL의 부분만 고려함을 의미합니다.

`On`인 경우 `ModMimeUsePathInfo` 매개 변수는 `mod_mime`가 *complete* URL을 기반으로 컨텐츠 유형을 결정하도록 지정합니다.즉, 가상 리소스는 확장자에 따라 적용되는 지표가 있습니다.
...
Dispatcher는 OpenSSL을 사용하여 HTTP를 통한 안전한 커뮤니케이션을 구현합니다. Dispatcher 버전 **4.2.0**&#x200B;부터, OpenSSL 1.0.0 및 OpenSSL 1.0.1이 지원됩니다. Dispatcher는 기본적으로 OpenSSL 1.0.0을 사용합니다. OpenSSL 1.0.1을 사용하려면 다음 절차를 사용하여 심볼릭 링크를 만들어 Dispatcher가 설치된 OpenSSL 라이브러리를 사용합니다.
...
>[!NOTE]
Apache의 사용자 정의 버전을 사용하는 경우, Apache와 Dispatcher가 동일한 버전의 [OpenSSL](https://www.openssl.org/source/)을 사용하여 컴파일되었는지 확인하십시오.
...
| 로그 수준 | 로그 파일에 메시지를 쓸 때의 로그 수준:<br/>**0** 오류 <br/>**1** 경고 <br/>**2** 정보 <br/>**3** 디버그 <br/>**참고:** 설치 및 테스트 중에는 로그 수준을 3로, 프로덕션 환경에서 실행할 때는 0으로 설정하는 것이 좋습니다. |
