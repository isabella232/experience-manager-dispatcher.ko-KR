...
예를 들어, 이 `dispatcher-apache2.4-linux-x86_64-ssl-4.3.1.tar.gz` 파일에는 Linux i686에서 실행되고 tar 형식을 사용하여 패키지되는 Apache 2.4 웹 서버용 Dispatcher 버전 4.3.1이 **포함되어** 있습니다.
...
* 현재 및 이전 릴리스의 문제를 나열하는 CHANGES 파일
...
Microsoft 인터넷 정보 시스템에 필요한 보관 파일:
...
| `disp_iis.ini` | IIS의 구성 파일입니다. 이 예는 사용자의 요구 사항에 따라 업데이트할 수 있습니다. **참고**:ini 파일의 이름 루트가 dll과 같아야 합니다. |
| `dispatcher.any` | Dispatcher의 예제 구성 파일입니다. |
| `author_dispatcher.any` | 작성자 인스턴스 작업을 하는 Dispatcher의 예제 구성 파일입니다. |
...
파일을 편집하여 Dispatcher `disp_iis.ini` 설치를 구성합니다. 파일의 기본 형식은 `.ini` 다음과 같습니다.
...
| 매개 변수 | 설명 |
|--- |--- |
| configupath | 로컬 파일 시스템 `dispatcher.any` 내의 위치(절대 경로). |
| 로그 파일 | 파일의 `dispatcher.log` 위치입니다. 이 설정이 안 되면 로그 메시지가 windows 이벤트 로그로 이동합니다. |
...
| 재허가 | HTTP 요청의 인증 헤더를 처리하는 방법을 지정합니다. 다음 값이 유효합니다.<br/>0 - 인증 헤더는 수정되지 않습니다. <br/>1 - &quot;Basic&quot; 이외의 &quot;Authorization&quot;이라는 헤더를 `Basic <IIS:LOGON\_USER>` 해당 헤더로 대체합니다.<br/> |
| servervariables | 서버 변수가 처리되는 방식을 정의합니다.<br/>0 - IIS 서버 변수가 Dispatcher와 AEM으로 전송되지 않습니다. <br/>1 - 모든 IIS 서버 변수(예: `LOGON\_USER, QUERY\_STRING, ...`)가 요청 헤더와 함께 Dispatcher로 전송되고, 캐시되지 않은 경우 AEM 인스턴스도 전송됩니다.  <br/>서버 변수에는 `AUTH\_USER, LOGON\_USER, HTTPS\_KEYSIZE` 및 기타 여러 가지가 포함됩니다. 변수의 전체 목록은 IIS 설명서를 참조하십시오. |
| enable_chunked_transfer | 클라이언트 응답에 대해 청크 전송을 활성화할지(1) 또는 비활성화할지(0) 여부를 정의합니다. 기본값은 0입니다. |
...
작성자 인스턴스의 기본 Flush 복제 에이전트가 플러쉬 요청으로 보안 자격 증명을 보내지 않도록 구성되었습니다. 따라서 Dispatcher 캐시를 사용하는 웹 사이트에서 익명 액세스를 허용해야 합니다.

웹 사이트에서 인증 방법을 사용하는 경우 이에 따라 Flush 복제 에이전트를 구성해야 합니다.

1. IIS 관리자를 열고 Dispatcher 캐시로 사용 중인 웹 사이트를 선택합니다.
1. 기능 보기 모드를 사용하는 경우 IIS 섹션에서 인증을 두 번 클릭합니다.
1. 익명 인증이 활성화되지 않으면 익명 인증을 선택하고 작업 영역에서 활성화를 클릭합니다.
...
1. 표시되는 대화 상자에서 ISAPI 및 CGI 제한 목록에 disp_iis.dll 라이브러리를 추가하려면 예를 클릭합니다.
   ...
   IIS 7.0 및 7.5의 경우 구성이 완료되었습니다. IIS 8.0을 구성하는 경우 나머지 단계를 계속 수행합니다.

1. (IIS 8.0) 처리기 매핑 목록에서 방금 만든 항목을 선택하고 [작업] 영역에서 [편집]을 클릭합니다.
1. (IIS 8.0) 스크립트 맵 편집 대화 상자에서 요청 제한 버튼을 클릭합니다.
1. (IIS 8.0) 아직 캐시되지 않은 파일 및 폴더에 대해 핸들러를 사용하려면 [요청이 매핑된 경우에만 처리기 호출]을 선택 해제하고 [확인]을 클릭합니다.
...
1. 발송자 캐시로 사용 중인 웹 사이트의 루트 폴더를 마우스 오른쪽 단추로 클릭하고 속성(예: 속성)을 클릭합니다 `C:\inetpub\wwwroot`.
1. 보안 탭에서 편집을 클릭한 다음 권한 대화 상자에서 추가를 클릭합니다. 사용자 계정을 선택하는 대화 상자가 열립니다. 위치 단추를 클릭하고 컴퓨터 이름을 선택한 다음 확인을 클릭합니다.

   다음 단계를 완료하는 동안 이 대화 상자를 엽니다.

1. IIS 관리자에서 디스패처 캐시에 사용할 IIS 사이트를 선택하고 창의 오른쪽에서 고급 설정을 클릭합니다.
1. 응용 프로그램 풀 속성 값을 선택하고 클립보드에 복사합니다.
1. 열린 대화 상자로 돌아갑니다. 선택할 개체 이름 입력 상자에 클립보드의 내용 `IIS AppPool\` 을 입력한 다음 붙여 넣습니다. 값은 다음 예제와 같아야 합니다.

   `IIS AppPool\DefaultAppPool`

1. 이름 확인 단추를 클릭합니다. Windows에서 사용자 계정을 확인하면 확인을 클릭합니다.
1. 발송자 폴더의 권한 대화 상자에서 방금 추가한 계정을 선택하고 전체 제어를 **제외한 계정에 대한 모든 권한을 활성화하고** 확인을 클릭합니다. 확인을 클릭하여 폴더 속성 대화 상자를 닫습니다.

### JSON MIME 유형 등록 - IIS 8.5 및 10 {#registering-the-json-mime-type-iis-and}

Dispatcher가 JSON 호출을 허용하도록 할 때 다음 절차를 사용하여 JSON MIME 유형을 등록하십시오.
...
1. 세그먼트를 `bin` 선택하고 제거를 클릭한 다음 확인 대화 상자에서 예를 클릭합니다.
...
1. Windows 탐색기를 사용하여 IIS 설치의 로그 폴더 `dispatcher` 아래에 명명된 폴더를 만듭니다. 일반 설치에 필요한 이 폴더의 경로는 다음과 같습니다 `C:\inetpub\logs\dispatcher`.
...
1. IIS 관리자에서 디스패처 캐시에 사용 중인 IIS 사이트를 선택하고 창의 오른쪽에서 고급 설정을 클릭합니다.
1. 응용 프로그램 풀 속성 값을 선택하고 클립보드에 복사합니다.
1. 열린 대화 상자로 돌아갑니다. 선택할 개체 이름 입력 상자에 클립보드의 내용 `IIS AppPool\` 을 입력한 다음 붙여 넣습니다. 값은 다음 예제와 같아야 합니다.
...
1. 이름 확인 단추를 클릭합니다. Windows에서 사용자 계정을 확인하면 확인을 클릭합니다.
1. 발송자 폴더의 권한 대화 상자에서 방금 추가한 계정을 선택하고 전체 제어를 **제외한 계정에 대한 모든 권한을 활성화한 다음** 확인을 클릭합니다. 확인을 클릭하여 폴더 속성 대화 상자를 닫습니다.
...
Dispatcher 사용을 시작하려면 먼저 알아야 할 사항이 있습니다.

* [발송자](dispatcher-configuration.md) 구성
* [AEM](page-invalidate.md) 을 Dispatcher와 함께 사용하도록 구성합니다.

## Apache 웹 서버 {#apache-web-server}

>[!CAUTION]
>
>Windows **및** Unix **에서** 설치에 대한 지침은 여기에 나와 있습니다. 단계 수행 시 주의하십시오.
...
>[!CAUTION]
>
>소스 파일을 컴파일하여 Apache 바이너리를 만드는 경우 **동적 모듈 지원을**&#x200B;켜십시오. 이 작업은 활성화 공유 **** 옵션 중 하나를 사용하여 수행할 수 있습니다. 적어도 `mod_so` 모듈이 포함됩니다.
>
>자세한 내용은 Apache Web Server 설치 설명서를 참조하십시오.

또한 Apache HTTP Server [보안 팁](https://httpd.apache.org/docs/2.4/misc/security_tips.html) 및 [보안 보고서를 참조하십시오](https://httpd.apache.org/security_report.html).
...
설치 아카이브 파일에는 다음과 같은 파일이 포함되어 있습니다. 이 파일은 Windows 또는 Unix를 선택했는지 여부에 따라 달라집니다.
...
1. Dispatcher 파일을 해당 Apache 모듈 디렉토리에 배치합니다.

   * **Windows**:장소 `disp_apache<x.y>.dll` `<APACHE_ROOT>/modules`
   * **Unix**:설치에 따라 `<APACHE_ROOT>/libexec` 또는 `<APACHE_ROOT>/modules`디렉토리를 찾습니다.\
      이 디렉터리 `dispatcher-apache<options>.so` 에 복사합니다.\
      장기 유지 관리를 간소화하려면 발송자에 이름이 지정된 상징적 링크 `mod_dispatcher.so` 를 만들 수도 있습니다.\
...
SELinux가 활성화된 RedHat Linux Kernel 2.6에서 Dispatcher를 실행하는 경우 디스패처 로그 파일에서 이와 같은 오류 메시지가 표시될 수 있습니다.

`Mon Jun 30 00:03:59 2013] [E] [16561(139642697451488)] Unable to connect to backend rend01 (10.122.213.248:4502): Permission denied`

이는 SELinux 보안 기능이 활성화되어 있기 때문일 수 있습니다. 그러면 다음 작업을 수행해야 합니다.

* 디스패처 모듈 파일의 SELinux 컨텍스트를 구성합니다.
* HTTPD 스크립트 및 모듈을 사용하여 네트워크 연결을 만듭니다.
* 캐시된 파일이 저장되는 문서의 SELinux 컨텍스트를 구성합니다.

Apache 웹 서버에 설치한 Dispatcher 모듈의 경로 `[path to the dispatcher.so file]` *`path to the docroot`* 와 문서 루트가 있는 경로(예: `/opt/cq/cache`):
...
Apache Web Server는 을 사용하여 구성해야 합니다 `httpd.conf`. Dispatcher 설치 키트에서 이름이 명명된 예제 구성 파일을 찾을 수 있습니다 `httpd.conf.disp<x>`.
...
1. 나열된 순서대로 다음 구성 항목을 추가해야 합니다.
   ...
   * **ModMimeUsePathInfo** 를 사용하여 mod_mime **의 동작을 구성합니다**.

1. (선택 사항) htdocs 디렉토리의 소유자를 변경하는 것이 좋습니다.

   * 하위 프로세스는 데몬으로 시작되지만 보안 목적으로 아파치 서버는 루트로 시작됩니다. DocumentRoot(`<APACHE_ROOT>/htdocs`)는 사용자 데몬에 속해야 합니다.
...
| Unix(심볼 링크 가칭) | `... LoadModule dispatcher_module libexec/mod_dispatcher.so ...` |
...
| 매개 변수 | 설명 |
|--- |--- |
| DispatcherConfig | Dispatcher 구성 파일의 위치 및 이름입니다. <br/>이 속성이 주 서버 구성에 있으면 모든 가상 호스트가 속성 값을 상속합니다. 그러나 가상 호스트에 DispatcherConfig 속성이 포함되어 주 서버 구성을 재정의할 수 있습니다. |
| DispatcherLog | 로그 파일의 위치 및 이름입니다. |
| DispatcherLogLevel | 로그 파일의 로그 수준: <br/>0 - 오류 <br/>1 - 경고 <br/>2 - 정보 <br/>3 - 디버그 <br/>**정보**:설치 및 테스트 중에 로그 수준을 3으로 설정한 다음 프로덕션 환경에서 실행할 때 0으로 설정하는 것이 좋습니다. |
| DispatcherNoServerHeader | *이 매개 변수는 더 이상 사용되지 않습니다.*<br/><br/> 사용할 서버 헤더를 정의합니다. <br/><ul><li>정의되지 않았거나 0 - HTTP 서버 헤더에 AEM 버전이 포함되어 있습니다. </li><li>1 - Apache server 헤더가 사용됩니다.</li></ul> |
| DispatcherDeclineRoot | 루트 &quot;/&quot;에 대한 요청을 거절할지 여부를 정의합니다. <br/>**0** - / <br/>**1** 에 대한 요청 수락 - 디스패처가 /에 대한 요청을 처리하지 않음올바른 매핑에 mod_alias를 사용하십시오. |
| DispatcherUseProcessedURL | Dispatcher의 모든 추가 처리에 사전 처리된 URL을 사용할지 여부를 정의합니다. <br/>**0** - 웹 서버에 전달된 원본 URL을 사용합니다. <br/>**1** - 디스패처는 디스패처 앞에 있는 핸들러에서 이미 처리된 URL을 사용합니다(예: `mod_rewrite`)을 클릭합니다.  예를 들어 원본 또는 처리된 URL이 발송자 필터와 일치합니다. 또한 URL은 캐시 파일 구조의 기초로 사용됩니다.   mod_rewrite에 대한 자세한 내용은 Apache 웹 사이트 설명서를 참조하십시오.예를 들어 Apache 2.4. mod_rewrite를 사용할 때는 &#39;passthrough&#39; 플래그를 사용하는 것이 좋습니다 | PT&#39;(다음 처리기로 통과)를 사용하여 다시 작성 엔진이 내부 request_rec 구조의 uri 필드를 파일 이름 필드의 값으로 설정하도록 합니다. |
| DispatcherPassError | ErrorDocument 처리에 대한 오류 코드를 지원하는 방법을 정의합니다. <br/>**0** - Dispatcher는 클라이언트에 대한 모든 오류 응답을 스풀합니다. <br/>**1** - Dispatcher는 클라이언트에 대한 오류 응답을 스풀링하지 않으며(상태 코드가 400보다 크거나 같은 경우) 상태 코드를 Apache로 전달합니다. Apache에서는 상태 코드를 처리합니다. 예를 들어 ErrorDocument 지시어가 이러한 상태 코드를 처리할 수 있습니다. <br/>**코드 범위** - Apache로 응답이 전달되는 오류 코드 범위를 지정합니다. 다른 오류 코드가 클라이언트에 전달됩니다. 예를 들어 다음 구성은 오류 412에 대한 응답을 클라이언트에 전달하며 다른 모든 오류가 Apache로 전달됩니다.DispatcherPassError 400-411,413-417 |
| DispatcherKeepAliveTimeout | 활성 유지 시간 초과(초)를 지정합니다. Dispatcher 버전 4.2.0부터 기본 keep-alive 값은 60입니다. 값 0을 지정하면 keep-alive가 비활성화됩니다. |
| DispatcherNoCanonURL | 이 매개 변수를 On으로 설정하면 기본 URL이 아닌 백 엔드에 전달되고 DispatcherUseProcessedURL의 설정이 무시됩니다. 기본값은 해제입니다. <br/>**참고**:발송자 구성의 필터 규칙은 항상 원시 URL이 아닌 정리된 URL에 대해 평가됩니다. |
...
AEM 버전(통계 목적)을 표시합니다. 헤더에서 이러한 정보를 사용할 수 없도록 하려면 다음을 설정할 수 있습니다. `  
ServerTokens Prod`\
자세한 내용은 [ServerTokens Directive(예: Apache 2.4)에 대한](https://httpd.apache.org/docs/2.4/mod/core.html) Apache 설명서를 참조하십시오.

**SetHandler**

이러한 항목 후에는 Dispatcher가 들어오는 요청을 처리하기 위해 **SetHandler** 문을 구성 컨텍스트(, `<Directory>``<Location>`)에 추가해야 합니다. 다음 예제에서는 Dispatcher가 전체 웹 사이트에 대한 요청을 처리하도록 구성합니다.
...
>[!NOTE]
이 **매개 변수는 모듈에 정의된 핸들러 이름이므로** SetHandler *문의 매개 변수를 위 예제와*&#x200B;정확하게작성해야 합니다.
이 명령에 대한 자세한 내용은 제공된 예제 구성 파일 및 Apache Web Server 설명서를 참조하십시오.
...
>[!NOTE]
매개 변수 `ModMimeUsePathInfo` 는 Dispatcher 버전 4.0.9 이상을 사용하는 경우에만 사용하고 구성해야 합니다.
(Dispatcher 버전 4.0.9는 2011년에 출시되었습니다. 이전 버전을 사용하는 경우 최신 Dispatcher 버전으로 업그레이드하는 것이 적절할 것입니다.)

ModMimeUsePathInfo **매개 변수** `On` 는 모든 Apache 구성에 대해 설정해야 합니다.

`ModMimeUsePathInfo On`

mod_mime 모듈(예: [Apache Module mod_mime](https://httpd.apache.org/docs/2.4/mod/mod_mime.html)참조)은 HTTP 응답으로 선택한 컨텐츠에 컨텐츠 메타데이터를 할당하는 데 사용됩니다. 기본 설정은 mod_mime이 컨텐츠 유형을 결정하는 경우 파일이나 디렉토리에 매핑되는 URL의 부분만 고려함을 의미합니다.

이 경우 매개 변수 `On`는 `ModMimeUsePathInfo` 전체 `mod_mime` URL을 기준으로 컨텐츠 유형을 결정하는 것을 ** 지정합니다.즉, 가상 리소스는 확장자에 따라 적용되는 지표가 있습니다.

다음 예제에서는 **ModMimeUsePathInfo를 활성화합니다**.
...
Dispatcher는 OpenSSL을 사용하여 HTTP를 통한 안전한 커뮤니케이션을 구현합니다. Dispatcher 버전 **4.2.0부터** OpenSSL 1.0.0 및 OpenSSL 1.0.1이 지원됩니다. Dispatcher는 기본적으로 OpenSSL 1.0.0을 사용합니다. OpenSSL 1.0.1을 사용하려면 다음 절차를 사용하여 심볼릭 링크를 만들어 Dispatcher가 설치된 OpenSSL 라이브러리를 사용합니다.
...
>[!NOTE]
Windows 및 Unix 환경을 위한 지침은 여기에 나와 있습니다.
...
### Sun Java System 웹 서버/iPlanet - 발송자 모듈 추가 {#sun-java-system-web-server-iplanet-add-the-dispatcher-module}
...
1. 웹 서버의 디렉토리에 Dispatcher 파일을 `plugin` 배치합니다.

### Sun Java System Web Server / iPlanet - 발송자에 대한 구성 {#sun-java-system-web-server-iplanet-configure-for-the-dispatcher}

을 사용하여 웹 서버를 구성해야 합니다 `obj.conf`. Dispatcher 설치 키트에서 이름이 명명된 예제 구성 파일을 찾을 수 있습니다 `obj.conf.disp`.

1. 다음으로 이동 `<WEBSERVER_ROOT>/config`.
1. 편집을 `obj.conf`위해 엽니다.
1. 시작하는 줄 복사:\
   `Service fn="dispService"`\
   의 초기화 섹션 `obj.conf.disp` 에서 을(를) 클릭합니다 `obj.conf`.
...
1. 시작하는 두 줄을 복사합니다.\
   ...
   및\
   `Init fn="dispInit"`\
   의 초기화 섹션 `obj.conf.disp` 에서 을(를) 클릭합니다 `magnus.conf`.

1. 변경 사항을 저장합니다.

>[!NOTE]
다음 구성은 모두 한 줄에 있어야 하며 `$(SERVER_ROOT)` 해당 값으로 `$(PRODUCT_SUBDIR)` 바꿔야 합니다.
...
| 로그 수준 | 로그 파일에 메시지를 쓸 때의 로그 수준: <br/>**0** 오류 <br/>**** 1 <br/>**경고** 2 <br/>**Infos** 3 <br/>**디버그 자습서 참고:** 설치 및 테스트 중에는 로그 수준을 3으로, 프로덕션 환경에서 실행할 때는 0으로 설정하는 것이 좋습니다. |
| keepalivetimeout | 활성 유지 시간 초과(초)를 지정합니다. Dispatcher 버전 4.2.0부터 기본 keep-alive 값은 60입니다. 값 0을 지정하면 keep-alive가 비활성화됩니다. |

사용자의 요구 사항에 따라 Dispatcher를 객체 서비스로 정의할 수 있습니다. 전체 웹 사이트에 대해 Dispatcher를 구성하려면 기본 개체를 수정합니다.
