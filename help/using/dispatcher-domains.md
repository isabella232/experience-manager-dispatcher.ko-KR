...
1. HTTP 요청은 웹 서버로 전송됩니다.
1. HTTP 요청은 Dispatcher로 전달됩니다.
1. Dispatcher는 캐시된 파일이 유효한지 여부를 결정합니다. 유효한 경우 캐시된 파일이 클라이언트에 제공됩니다.
1. 캐시된 파일이 유효하지 않은 경우 디스패처는 AEM 게시 인스턴스에서 새로 렌더링된 페이지를 요청합니다.
...
1. Dispatcher는 하나 이상의 .stat 파일을 업데이트하여 캐시된 파일을 무효화합니다.
...
* Dispatcher Flush 복제 에이전트는 AEM 게시 인스턴스에 구성됩니다. (게시 [인스턴스에서 발송자 캐시 무효화를 참조하십시오](page-invalidate.md).)
...
## AEM에서 들어오는 URL을 다시 씁니다. {#aem-rewrites-incoming-urls}
...
* 각 가상 도메인에는 발송자 설치 페이지에 설명된 대로 발송자 관련 구성도 [포함되어야](dispatcher-install.md) 합니다.
...
### 여러 도메인을 처리하도록 디스패처 구성 {#configure-dispatcher-to-handle-multiple-domains}
...
* 이 `/virtualhosts` 속성은 도메인 이름으로 설정됩니다. 이 속성을 사용하면 디스패처가 팜을 도메인과 연결할 수 있습니다.
* 이 `/filter` 속성을 사용하면 도메인 이름 부분 다음에 잘리는 요청 URL의 경로에 액세스할 수 있습니다. 예를 들어 URL의 `https://branda.com/en.html` 경우 경로는 해석되므로 `/en.html`필터가 이 경로에 대한 액세스를 허용해야 합니다.

* 이 `/docroot` 속성은 Dispatcher 캐시에서 도메인 사이트 컨텐츠의 루트 디렉토리 경로로 설정됩니다. 이 경로는 원래 요청에서 연결된 URL의 접두사로 사용됩니다. 예를 들어, 의 `/usr/lib/apache/httpd-2.4.3/htdocs/sitea` docroot를 사용하면 요청이 `https://branda.com/en.html` 파일로 `/usr/lib/apache/httpd-2.4.3/htdocs/sitea/en.html` 확인됩니다.
...
* 이 `/docroot` 속성은 웹 서버의 기본 문서 루트로 설정됩니다. 일반적으로 이 디렉토리는 폴더를 `/content` 만드는 디렉토리입니다. Linux에서 Apache의 예 값은 입니다 `/usr/lib/apache/httpd-2.4.3/htdocs`.
* 이 `/filter` 속성을 사용하면 `/content` 디렉토리 아래 파일에 액세스할 수 있습니다.
...
### 리소스 해상도에 대한 Sling 매핑 구성 {#configure-sling-mapping-for-resource-resolution}
...
Sling 리소스 매핑에 대한 자세한 내용은 Sling [설명서의 리소스](https://sling.apache.org/site/mappings-for-resource-resolution.html) 해상도에 대한 매핑을 참조하십시오.
...
| `/etc/map/http/branda.com/libs` | sling:Mapping | 이름:sling:internal리디렉션 <br/>유형:문자열 <br/>값:/libs |
...
## Dispatcher Flush 복제 에이전트 구성 {#configuring-the-dispatcher-flush-replication-agent}

AEM 게시 인스턴스의 Dispatcher Flush 복제 에이전트가 올바른 Dispatcher 팜에 무효화 요청을 보내야 합니다. 팜을 타깃팅하려면 [전송] 탭에서 Dispatcher Flush 복제 에이전트의 URI 속성을 사용합니다. 캐시를 무효화하도록 구성된 Dispatcher 팜의 `/virtualhost` 속성 값을 포함합니다.
...
## 웹 서버가 들어오는 URL을 다시 씁니다. {#the-web-server-rewrites-incoming-urls}
...
## 웹 서버에서 가상 호스트 정의 및 다시 쓰기 규칙 {#define-virtual-hosts-and-rewrite-rules-on-the-web-server}
...
* 각 가상 도메인에는 발송자 설치 페이지에 설명된 대로 발송자 관련 구성도 [포함되어야](dispatcher-install.md) 합니다.
* Dispatcher 모듈은 웹 서버가 다시 작성한 URL을 사용하도록 구성해야 합니다. (Dispatcher 설치에서 `DispatcherUseProcessedURL` 속성을 [참조하십시오](dispatcher-install.md).)
...
* 이 `DispatherUseProcessedURL` 속성은 로 설정됩니다 `1`. (10행)
...
### 디스패처 팜 구성 {#configure-a-dispatcher-farm}

웹 서버가 URL을 다시 작성할 때 Dispatcher에는 Dispatcher 구성에 따라 정의된 단일 팜이 [필요합니다](dispatcher-configuration.md). 웹 서버 가상 호스트 및 URL 이름 바꾸기 규칙을 지원하려면 다음 구성이 필요합니다.

* 이 `/virtualhosts` 속성에는 모든 VirtualHost 정의에 대한 ServerName 값이 포함되어야 합니다.
...
* 이 `/virtualhosts` 속성을 통해 Dispatcher가 `brandA.com` 및 `brandB.com` 도메인에 대한 요청을 처리합니다. (line 12)
* 이 `/statfileslevel` 속성은 2로 설정되므로 도메인의 웹 컨텐츠(41줄)가 포함된 각 디렉토리에 통계 파일이 만들어집니다. `/statfileslevel "2"`
...
웹 서버 컨텍스트에서 리소스 경로가 올바로 확인되지 않으면 참조를 다시 작성합니다. 예를 들어 이미지 생성 구성 요소가 /content/sitea/en/products.navimage.png와 같은 링크를 만들 때는 변압기가 필요합니다. 완전한 기능을 갖춘 인터넷 웹 [사이트를 만드는 방법](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/the-basics.html) (How to Create a Fully Features Internet Website)의 위쪽 구성 요소는 이러한 링크를 만듭니다.

Sling [리라이터는](https://sling.apache.org/documentation/bundles/output-rewriting-pipelines-org-apache-sling-rewriter.html) Sling 출력을 사후 처리하는 모듈입니다. 리필터의 SAX 파이프라인 구현은 생성기, 하나 이상의 트랜스포머 및 직렬 변환기로 구성됩니다.

* **** 생성기:Sling 출력 스트림(HTML 문서)을 구문 분석하고 특정 요소 유형이 발견되면 SAX 이벤트를 생성합니다.
* **** 변환기:SAX 이벤트를 수신하여 이벤트 대상(HTML 요소)을 수정합니다. 리필터 파이프라인은 0개 이상의 변압기를 포함합니다. 변압기는 차례로 실행되어 SAX 이벤트를 시퀀스의 다음 변압기로 전달합니다.
* **** 직렬 변환기:각 변압기의 수정 사항을 포함하여 출력을 일련화합니다.

![](assets/chlimage_1-15.png)

### AEM 기본 리작성기 파이프라인 {#the-aem-default-rewriter-pipeline}

AEM에서는 텍스트/html 유형의 문서를 처리하는 기본 파이프라인 리기록기를 사용합니다.

* 이 생성기는 HTML 문서를 구문 분석하여 a, img, area, form, base, link, script 및 body 요소가 나타나면 SAX 이벤트를 생성합니다. 생성기 별칭은 `htmlparser`입니다.
* 이 파이프라인에는 다음과 같은 변압기가 포함되어 있습니다. `linkchecker``mobile`, `mobiledebug`3 `contentsync`. 변환기는 참조된 HTML 또는 HTM 파일에 대한 경로를 외부화하여 링크가 끊기지 않도록 합니다. `linkchecker`
* 직렬 변환기가 HTML 출력을 씁니다. 직렬 변환기 별칭은 htmlwriter입니다.

노드는 `/libs/cq/config/rewriter/default` 파이프라인을 정의합니다.

### 변환기 만들기 {#creating-a-transformer}

다음 작업을 수행하여 변환기 구성 요소를 만들고 파이프라인에서 사용합니다.

1. 인터페이스를 `org.apache.sling.rewriter.TransformerFactory` 구현합니다. 이 클래스는 변압기 클래스의 인스턴스를 만듭니다. 속성 값(변환기 별칭)을 `transformer.type` 지정하고 클래스를 OSGi 서비스 구성 요소로 구성합니다.
1. 인터페이스를 `org.apache.sling.rewriter.Transformer` 구현합니다. 작업을 최소화하려면 `org.apache.cocoon.xml.sax.AbstractSAXPipe` 클래스를 확장할 수 있습니다. startElement 메서드를 재정의하여 다시 작성 동작을 사용자 지정합니다. 이 메서드는 변환기로 전달되는 모든 SAX 이벤트에 대해 호출됩니다.
...
>
>* 속성을 로 `pipeline.mode` 설정합니다 `global`.
>* 속성을 `service.ranking` 양의 정수로 설정합니다.
>* 속성을 포함하지 `pipeline.type` 마십시오.


>[!NOTE]
>
>Content Package Maven 플러그인의 [다중 모듈](https://helpx.adobe.com/experience-manager/aem-previous-versions.html) 원형 유형을 사용하여 Maven 프로젝트를 만듭니다. POM 파섹

다음 예제에서는 이미지 파일에 대한 참조를 다시 쓰는 변환기를 구현합니다.

* MyRewriterTransformerFactory 클래스는 MyRewriterTransformer 개체를 인스턴스화합니다. pipeline.type 속성은 변환기 별칭을 mytransformer로 설정합니다. 파이프라인에 별칭을 포함하려면 파이프라인 구성 노드에 트랜스포머 목록에 이 별칭이 포함됩니다.
* MyRewriterTransformer 클래스는 AbstractSXTransformer 클래스의 startElement 메서드를 재정의합니다. startElement 메서드는 img 요소의 src 속성 값을 다시 씁니다.

이러한 예는 견고하지 않으며 제작 환경에서 사용해서는 안 됩니다.
...
### 리작성기 파이프라인에 변환기 추가 {#adding-the-transformer-to-a-rewriter-pipeline}

변압기를 사용하는 파이프라인을 정의하는 JCR 노드를 생성합니다. 다음 노드 정의는 텍스트/html 파일을 처리하는 파이프라인을 만듭니다. HTML용 기본 AEM 생성기 및 파서가 사용됩니다.

>[!NOTE]
>
>Transformer 속성을 `pipeline.mode` 로 `global`설정하면 파이프라인을 구성할 필요가 없습니다. 이 `global` 모드는 변압기를 모든 파이프라인에 삽입합니다.

### 재기록기 구성 노드 - XML 표현 {#rewriter-configuration-node-xml-representation}
