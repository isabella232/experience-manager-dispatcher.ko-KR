...
>[!NOTE]
>
>다음 설정에 유의하십시오 `/enableTTL` to 1은 디스패처 쪽에서만 TTL 캐싱을 활성화합니다. 따라서 추가 파일(위 참조)에 포함된 TTL 정보는 디스패처에서 이러한 파일 유형을 요청하는 다른 사용자 에이전트에게 제공되지 않습니다. CDN 또는 브라우저와 같은 다운스트림 시스템에 캐싱 헤더를 제공하려면 다음을 구성해야 합니다 `/cache/headers` 섹션에 알맞게 지정할 수 있습니다.
...
>
>그러나 해당 수준에는 추가 리소스가 필요합니다. Dispatcher가 원활하게 작동할 때 *요구 사항에 따라* 로그 수준을 낮출 수 있습니다.
