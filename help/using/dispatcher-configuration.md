...
설정되면 `/enableTTL` 속성은 백엔드의 응답 헤더를 평가하고 property will evaluate the response headers from the backend, and if they contain a `Cache-Control` 최대 기간 또는 `Expires` 날짜가 포함된 경우 캐시 파일 옆에 수정 시간이 만료 날짜와 동일한 보조 빈 파일이 생성됩니다. 캐시된 파일이 수정 시간 이후에 요청되면 백엔드에서 자동으로 다시 요청됩니다.
