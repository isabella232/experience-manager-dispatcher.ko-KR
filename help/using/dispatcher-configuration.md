...
* 구성 파일이 큰 경우 여러 개의 작은 파일로 분할하고(관리가 더 쉬워짐) 각 파일을 포함할 수 있습니다.
* 자동으로 생성된 파일을 포함합니다.
...
포함할 파일 범위를 지정하려면 별표(`*`)을 와일드카드로 사용할 수 있습니다.
...
예를 들어 dispatcher.any 파일이 캐시 디렉토리와 동일한 디렉토리에 있는 경우 [docroot](#specifying-the-cache-directory) 속성을 사용할 수 있습니다.
...
* 캐싱할 파일 및 캐싱할 위치와 같은 몇 가지 다른 동작입니다.

이 값에는 영숫자(a-z, 0-9)가 포함될 수 있습니다. 다음 예제는 `/daycom` 및 `/docsdaycom`라는 두 팜에 대한 스켈레톤 정의를 보여 줍니다.
...
>[!NOTE]
>
>둘 이상의 렌더링 팜을 사용하는 경우 목록은 상향식으로 평가됩니다. 이 흐름은 [가상 호스트](#identifying-virtual-hosts-virtualhosts) 사용.
...
예를 들어 게시 인스턴스에 대한 페이지 활성화 요청을 처리하는 Dispatcher 인스턴스는 `/clientheaders` 섹션에 `PATH` 헤더가 필요합니다. 다음 `PATH` 헤더를 사용하면 복제 에이전트와 Dispatcher 간에 통신할 수 있습니다.
...
다음 예제 구성은 `.com` 및 `.ch` myCompany의 도메인 및 mySubDivision의 모든 도메인:
...
* 없는 경우 `virtualhosts` 값값 `scheme` 및 `uri` 둘 다 일치하는 부품 `scheme` 및 `uri` 요청의 첫 번째 발생 가상 호스트가 `host` 요청 이 사용됩니다.
...
>[!CAUTION]
>
> 디렉토리 하위 매개변수를 구성할 때 **포함하지 않음** 루트 폴더를 가리킵니다(`/directory "/"`) 심각한 문제를 일으킬 수 있기 때문입니다. 항상 세션 정보를 저장하는 폴더의 경로를 지정합니다. 예:
...
응답이 허용되는 시간(밀리초)을 지정합니다. 기본값은 `"600000"`이므로 Dispatcher가 10분 동안 대기합니다. 설정 `"0"` 시간 제한 을 제거합니다.

응답 헤더를 구문 분석하는 동안 시간 제한에 도달하면 HTTP 상태 504(잘못된 게이트웨이)가 반환됩니다. 응답 본문을 읽는 동안 시간 초과에 도달하면 Dispatcher는 불완전한 응답을 클라이언트에 반환합니다. 또한 기록되었을 수 있는 모든 캐시 파일도 삭제합니다.
...
만약 `/secure` 속성에 값이 `"1"`, Dispatcher는 HTTPS를 사용하여 AEM 인스턴스와 통신합니다. 자세한 내용은 [SSL을 사용하도록 Dispatcher 구성](dispatcher-ssl.md#configuring-dispatcher-to-use-ssl)도 참조하십시오.

**/always-resolve**

Dispatcher 버전 **4.1.6**&#x200B;에서는 `/always-resolve` 속성을 다음과 같이 구성할 수 있습니다.

* 로 설정된 경우 `"1"`를 지정하면 모든 요청에서 호스트 이름을 확인합니다(Dispatcher가 IP 주소를 캐시하지 않음). 각 요청에 대한 호스트 정보를 가져오는 데 필요한 추가 호출로 인해 성능에 약간의 영향이 있을 수 있습니다.
* 이 속성을 설정하지 않으면 기본적으로 IP 주소가 캐시됩니다.
...
다음 `/filter` 섹션은 HTTP 요청의 요청 라인 부분에 있는 패턴에 따라 콘텐츠에 대한 액세스를 거부하거나 허용하는 일련의 규칙으로 구성됩니다. 다음허용 목록에 추가하다에 대한 전략 사용 `/filter` 섹션:
...
* **요청 라인의 고급 요소:** Dispatcher 4.2.0부터 4개의 새로운 필터 요소를 사용할 수 있습니다. 이러한 새 요소는 다음과 같습니다 `/path`, `/selectors`, `/extension`, 및 `/suffix` 각각 사용할 수 있습니다. URL 패턴을 추가로 제어하려면 이러한 항목 중 하나 이상을 포함하십시오.
...
패턴은 요청 줄과 의 공백 문자를 고려해야 합니다. `<CRLF>` 자.
...
필터가 예상한 방식으로 트리거되지 않으면 을(를) 활성화합니다 [추적 로깅](#trace-logging) Dispatcher에서 요청을 가로채는 필터를 확인할 수 있습니다.
...
필터를 사용하면 ASP 페이지 및 게시 인스턴스 내의 중요 영역 등 다양한 요소에 대한 액세스를 거부할 수도 있습니다. 다음 필터는 ASP 페이지에 대한 액세스를 거부합니다.
...
제한된 영역 내의 단일 페이지에 액세스해야 하는 경우 해당 페이지에 대한 액세스를 허용할 수 있습니다. 예를 들어 워크플로 콘솔 내의 보관 탭에 대한 액세스를 허용하려면 다음 섹션을 추가합니다.
...
>[!NOTE]
>
>여러 필터 패턴이 요청에 적용되면 마지막으로 적용된 필터 패턴이 효과를 냅니다.
...
아래는 다음에서 컨텐츠를 수집하는 규칙을 보여 줍니다 `/content` 경로, 선택기 및 확장에 필터를 사용하는 경로 및 해당 하위 트리:
...
Dispatcher를 구성할 때는 외부 액세스를 가능한 한 제한해야 합니다. 다음 예제에서는 외부 방문자에게 최소한의 액세스 권한을 제공합니다.

* `/content`
* 디자인 및 클라이언트 라이브러리와 같은 기타 컨텐츠. 예:

   * `/etc/designs/default*`
   * `/etc/designs/mydesign*`

필터를 만들면 [페이지 액세스 테스트](#testing-dispatcher-security) AEM 인스턴스가 안전한지 확인합니다.

`dispatcher.any` 파일의 다음 `/filter` 섹션은 [Dispatcher 구성 파일](#dispatcher-configuration-files)의 기초로 사용할 수 있습니다.

이 예제는 Dispatcher와 함께 제공되는 기본 구성 파일을 기반으로 하며 프로덕션 환경에서 사용하기 위한 목적으로 계획되었습니다. 접두어가 있는 항목 `#` 비활성화되었습니다(주석 처리됨). 이러한 항목을 활성화하려는 경우( `#` (해당 행). 그렇게 하면 보안에 영향을 줄 수 있습니다.

모든 항목에 대한 액세스를 거부한 다음 특정(제한된) 요소에 대한 액세스를 허용합니다.
...
* 에 대한 외부 액세스 비활성화 `/admin` cq 버전 5.4 또는 이전 버전을 사용 중인 경우
...
>[!CAUTION]
>
>콘솔 및 디렉터리에 대한 액세스는 프로덕션 환경에 보안 위험을 초래할 수 있습니다. 명시적인 정당성이 없는 경우 비활성화된 상태로 유지되어야 합니다(주석 처리됨).

>[!CAUTION]
>
>만약 [게시 환경에서 보고서 사용](https://experienceleague.adobe.com/docs/experience-manager-65/administering/operations/reporting.html?lang=ko-KR#using-reports-in-a-publish-environment), Dispatcher가 액세스 권한을 거부하도록 구성해야 합니다. `/etc/reports` 외부 방문자용.
...
>[!NOTE]
>
>규칙에 `/query`, 쿼리 문자열을 포함하고 제공된 쿼리 패턴과 일치하는 요청만 일치합니다.
>
...
에 대한 일반 페이지 렌더링이 표시됩니다 `/content/add_valid_page.html?debug=layout`.
...
익명 쓰기 액세스 사용 여부를 확인하려면 터미널 또는 명령 프롬프트에서 다음 명령을 실행하십시오. 노드에 데이터를 쓸 수 없어야 합니다.

`curl -X POST "https://anonymous:anonymous@hostname:port/content/usergenerated/mytestnode"`

Dispatcher 캐시를 무효화하고 코드 403 응답을 수신하는지 확인하려면 터미널 또는 명령 프롬프트에서 다음 명령을 실행하십시오.
...
신디케이션 요청은 Dispatcher만을 위한 것이므로 기본적으로 렌더러(예: AEM 인스턴스)에 전송되지 않습니다.
...
>웹 서버는 Dispatcher 캐시 파일을 사용할 때 올바른 상태 코드를 전달해야 하므로 찾을 수도 있는 것이 중요합니다.
...
* 유형은 `glob` 패턴과 일치하는 문서를 캐시할지 여부를 나타냅니다. 값은 `allow` (문서를 캐시하려면) 또는 `deny` (항상 문서를 렌더링하기 위해)

동적 페이지가 없는 경우(위의 규칙에 의해 이미 제외된 페이지 이상) Dispatcher가 모든 항목을 캐싱하도록 구성할 수 있습니다. 규칙 섹션은 다음과 같습니다.
...
>[!NOTE]
>
>캐시된 페이지에 대한 사용자 권한을 확인하지 않으므로 닫힌 사용자 그룹을 캐시하지 마십시오.
...
* 특정 수준의 파일이 무효화되면 **모두** `.stat` docroot의 파일 **to** 무효화된 파일 또는 구성된 파일의 수준 `statsfilevel` (더 작은 것 중)를 터치합니다.

   * 예: 만약 `statfileslevel` 속성이 6에 추가되고, 파일이 레벨 5에서 무효화됩니다. `.stat` docroot에서 5로 파일을 터치합니다. 이 예제를 계속 진행하면 파일이 7레벨에서 무효화된 경우 `stat` docroot에서 6까지 파일을 수정(다음 `/statfileslevel = "6"`).
...
파일이 `/content/myWebsite/xx` 이 무효화된 경우 `.stat` docroot에서 다음으로 파일 `/content/myWebsite/xx`이 터치되었습니다. 이 시나리오는 `/content/myWebsite/xx` 예를 들어 `/content/myWebsite/yy` 또는 `/content/anotherWebSite`.

>[!NOTE]
>
>추가 헤더를 보내 무효화를 방지할 수 있습니다 `CQ-Action-Scope:ResourceOnly`. 이 방법은 캐시의 다른 부분을 무효화하지 않고 특정 리소스를 플러시하는 데 사용할 수 있습니다. 자세한 내용은 [이 페이지](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html) 및 [수동으로 Dispatcher 캐시 무효화](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/page-invalidate.html?lang=ko-KR#configuring)를 참조하십시오.
...
자동 무효화를 사용하면 Dispatcher는 콘텐츠 업데이트 후 캐시된 파일을 삭제하지 않지만 다음에 요청할 때 유효성을 확인합니다. 자동 무효화되지 않은 캐시의 문서는 콘텐츠 업데이트가 명시적으로 삭제될 때까지 캐시에 남아 있습니다.
...
* `/invalidate` 구성과 일치하는 다른 모든 파일은 즉시 삭제되지 않습니다. 이러한 파일은 다음 요청이 발생할 때 삭제됩니다. 예제에서는 `/content/wknd.html` 는 삭제되지 않습니다. 다음 경우에 삭제됩니다 `/content/wknd.html` 가 요청됩니다.

다운로드할 자동으로 생성된 PDF 및 ZIP 파일을 제공하는 경우 이러한 파일도 자동으로 무효화해야 할 수 있습니다. 구성 예는 다음과 같습니다.
...
이 방법은 여러 가지 사용 사례를 다루는 데 사용할 수 있습니다. 예를 들어, 다른 애플리케이션별 캐시를 무효화하거나, 페이지의 외부 URL과 docroot의 해당 위치가 컨텐츠 경로와 일치하지 않는 경우를 처리합니다.
...
>
>이 작업이 수행되지 않으면 모든 클라이언트가 캐시를 지우도록 호출을 실행할 수 있습니다. 반복적으로 수행하는 경우 사이트 성능에 심각한 영향을 줄 수 있습니다.
...
>[!NOTE]
>
>허용 목록 방식으로 `ignoreUrlParams` 설정을 구성하는 것이 좋습니다. 이에 따라 모든 쿼리 매개변수는 무시되고 알려진 쿼리 매개변수 또는 예상 쿼리 매개변수만 무시되는 항목에서 제외(“거부”)됩니다. 자세한 내용 및 예는 다음을 참조하십시오 [이 페이지](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-its-ignoreurlparams-rules-configured-in-an-allow-list-manner).
...
>[!NOTE]
>
>glob 속성을 구성할 때 쿼리 매개 변수 이름과 일치해야 합니다. 예를 들어 다음 URL `http://example.com/path/test.html?p1=test&p2=v2`에서 “p1” 매개변수를 무시하려는 경우 glob 속성은 다음과 같아야 합니다.
> `/0002 { /glob "p1" /type "allow" }`

다음 예제에서는 Dispatcher가 `nocache` 매개변수를 제외하고 다른 모든 매개변수를 무시하게 됩니다. 따라서 를 포함하는 요청 URL은 `nocache` 매개 변수는 Dispatcher에서 캐시하지 않습니다.
...
>[!NOTE]
>
>이 기능은 버전에서 사용할 수 있습니다 **4.1.11** Dispatcher에 속해 있어야 합니다.
...
>[!NOTE]
>
>파일 글로벌 문자를 사용할 수 없습니다. 자세한 내용은 [glob 속성에 대한 패턴 디자인](#designing-patterns-for-glob-properties)을 참조하십시오.
...
>* 다음을 추가합니다 [Apache 지시문](https://httpd.apache.org/docs/2.4/mod/core.html#fileetag) 디스패처 관련 섹션에서 다음을 수행합니다.
>
>```xml
>FileETag none
>```
...
기본값은 입니다. `0755` 소유자가 읽기, 쓰기 또는 검색과 그룹 및 다른 사용자가 읽거나 검색할 수 있도록 합니다.

### .stat 파일 터치 조절 {#throttling-stat-file-touching}

기본 `/invalidate` 속성을 사용하면 모든 활성화가 모든 `.html` 파일을 효과적으로 무효화합니다(해당 경로가 `/invalidate` 섹션과 일치하는 경우). 트래픽이 많이 발생하는 웹 사이트에서 여러 개의 후속 활동을 수행하면 백엔드의 cpu 로드가 증가합니다. 이런 시나리오에서는 &#39;스로틀&#39;이 바람직하다 `.stat` 웹 사이트를 반응형으로 유지하기 위해 파일을 터치합니다. 를 사용하여 이 작업을 수행할 수 있습니다 `/gracePeriod` 속성을 사용합니다.

다음 `/gracePeriod` 속성은 마지막으로 발생한 활성화 후에도 오래된 자동 무효화된 리소스가 캐시에서 계속 제공될 수 있는 시간(초)을 정의합니다. 이 속성은 활성화 일괄 처리가 전체 캐시를 반복적으로 무효화하는 설정에서 사용할 수 있습니다. 권장 값은 2초입니다.
...
시간 기반 캐시 무효화는 `/enableTTL` 속성 및 HTTP 표준의 일반 만료 헤더가 있는지 확인합니다. 속성을 1(`/enableTTL "1"`) 백엔드의 응답 헤더를 평가합니다. 헤더에 `Cache-Control`, `max-age` 또는 `Expires` 날짜, 캐시된 파일 옆에 있는 보조 빈 파일이 생성되며 수정 시간은 만료 날짜와 같습니다. 수정 시간 이후에 캐시된 파일이 요청되면 백엔드에서 자동으로 다시 요청됩니다.

Dispatcher 4.3.5 이전에는 TTL 무효화 로직이 구성된 TTL 값만 기반으로 했습니다. Dispatcher 4.3.5를 사용하면 두 TTL이 모두 설정됩니다 **및** dispatcher 캐시 무효화 규칙이 고려됩니다. 따라서 캐시된 파일의 경우:

1. `/enableTTL`을(를) 1로 설정하면 파일 만료가 확인됩니다. 설정된 TTL에 따라 파일이 만료된 경우 다른 검사는 수행되지 않으며 캐시된 파일이 백엔드에서 다시 요청됩니다.
2. 파일이 만료되지 않은 경우 또는 `/enableTTL` 가 구성되어 있지 않으면, 표준 캐시 무효화 규칙이 적용되는 예: [/statfileslevel](#invalidating-files-by-folder-level) 및 [/invalidate](#automatically-invalidating-cached-files). 이 흐름은 Dispatcher가 TTL이 만료되지 않은 파일을 무효화할 수 있음을 의미합니다.

이 새 구현은 파일의 TTL이 더 길지만(예: CDN에서) TTL이 만료되지 않은 경우에도 여전히 무효화될 수 있는 사용 사례를 지원합니다. 이는 Dispatcher의 캐시 적중 비율보다 컨텐츠 새로 고침을 선호합니다.

반대로 필요할 경우 **전용** 파일에 적용된 만료 논리를 설정한 다음 설정합니다. `/enableTTL` 을 1로 설정하고 표준 캐시 무효화 메커니즘에서 해당 파일을 제외합니다. 예를 들어 다음 작업을 수행할 수 있습니다.

* 파일을 무시하려면 [무효화 규칙](#automatically-invalidating-cached-files) 캐시 섹션에 있습니다. 아래의 코드 조각에서 `.example.html` 는 무시되고 설정된 TTL이 전달된 경우에만 만료됩니다.
...
* 파일이 자동으로 무효화되지 않도록 높은 [/statfilelevel](#invalidating-files-by-folder-level)을(를) 설정할 수 있는 방식으로 콘텐츠 구조를 설계합니다.

이렇게 하면 `.stat` 파일 무효화가 사용되지 않고 지정된 파일에 대해 TTL 만료만 활성화됩니다.
...
렌더링 카테고리에 대한 점수는 이전 응답 시간과 Dispatcher가 시도하는 이전 실패 및 성공한 연결을 기반으로 합니다. 각 시도에 대해, 요청된 URI의 범주에 대한 점수가 업데이트됩니다.
...
URI의 범주를 결정하기 위해 Dispatcher는 일치하는 항목이 발견될 때까지 URI를 각 범주 패턴과 비교합니다. Dispatcher는 목록의 첫 번째 카테고리에서 시작되고 순서대로 계속됩니다. 따라서 보다 구체적인 패턴을 가진 범주를 먼저 배치하십시오.
...
다음 `/stickyConnectionsFor` 속성은 고정 문서를 포함하는 하나의 폴더를 정의합니다. 이 속성은 URL을 사용하여 액세스합니다. Dispatcher는 이 폴더에 있는 단일 사용자로부터 동일한 렌더링 인스턴스로 모든 요청을 보냅니다. 고정 연결은 세션 데이터가 존재하는지 확인하고 모든 문서에 대해 일관성을 유지합니다. 이 메커니즘은 `renderid` 쿠키를 사용합니다.
...
고정 연결이 활성화되면 Dispatcher 모듈은 `renderid` 쿠키 . 이 쿠키에는 `httponly` 보안을 강화하기 위해 추가해야 하는 플래그입니다. 을(를) 추가합니다 `httponly` 플래그를 설정하여 `httpOnly` 속성( `/stickyConnections` 노드 `dispatcher.any` 구성 파일. 속성 값(`0` 또는 `1`)은 `renderid` 쿠키에 `HttpOnly` 속성이 추가되었는지 여부를 정의합니다. 기본값은 입니다. `0`를 의미하는 것은 속성이 추가되지 않음을 의미합니다.

`httponly` 플래그에 대한 추가 정보는 [이 페이지](https://www.owasp.org/index.php/HttpOnly)를 참조하십시오.

### secure {#secure}

고정 연결이 활성화되면 Dispatcher 모듈은 `renderid` 쿠키 . 이 쿠키에는 `secure` 보안을 강화하기 위해 추가해야 하는 플래그입니다. 을(를) 추가합니다 `secure` 플래그 설정 `secure` 속성( `/stickyConnections` 노드 `dispatcher.any` 구성 파일. 속성 값(`0` 또는 `1`)은 `renderid` 쿠키에 `secure` 속성이 추가되었는지 여부를 정의합니다. 기본값은 입니다. `0`를 반환해야 하며, 이것은 속성이 추가됨을 의미합니다 **if** 들어오는 요청이 안전합니다. 값이 `1`로 설정되면 수신 요청의 보안 여부와 관계없이 보안 플래그가 추가됩니다.
...
`/health_check` 속성을 사용하여 500 상태 코드가 발생할 때 검사되는 URL을 지정합니다. 이 페이지가 500 상태 코드도 반환하는 경우 인스턴스를 사용할 수 없으며 구성 가능한 시간 벌칙인 것으로 간주됩니다( `/unavailablePenalty`)가 렌더링에 적용되어 다시 시도하기 전에 적용됩니다.
...
원래 요청이 실패할 때 다른 렌더링으로 요청을 다시 전송하려면 Dispatcher 팜에서 페일오버 메커니즘을 활성화합니다. 장애 조치가 활성화된 경우 Dispatcher는 다음과 같은 동작을 합니다.
...
>[!NOTE]
>
>본문이 포함된 HTTP 요청을 다시 시도하려면 Dispatcher는 실제 콘텐츠 스풀링하기 전에 `Expect: 100-continue` 요청 헤더를 렌더링으로 보냅니다. CQSE가 있는 CQ 5.5는 즉시 100(CONTINUE) 또는 오류 코드로 응답합니다. 다른 서블릿 컨테이너도 지원됩니다.

## 중단 오류 무시 - /ignoreEINTR {#ignoring-interruption-errors-ignoreeintr}

>[!CAUTION]
>
>이 옵션은 필요하지 않습니다. 다음 로그 메시지가 표시되는 경우에만 사용합니다.
>
>`Error while reading response: Interrupted system call`

모든 파일 시스템 중심 시스템 호출 중단 가능 `EINTR` 시스템 호출 객체가 NFS를 통해 액세스하는 원격 시스템에 있는 경우 이러한 시스템 호출이 시간 초과되거나 중단될 수 있는지 여부는 기본 파일 시스템이 로컬 시스템에 마운트된 방식에 따라 결정됩니다.
...
이러한 인터럽트를 무시하려면 다음 매개 변수를 `dispatcher.any` (이전) `/farms`):
...
를 사용하는 경우 **Apache** 웹 서버에서는 회전된 로그 또는 파이프 로그 또는 둘 다에 표준 기능을 사용할 수 있습니다. 예를 들어 파이프된 로그를 사용하는 경우는 다음과 같습니다.

`DispatcherLog "| /usr/apache/bin/rotatelogs logs/dispatcher.log%Y%m%d 604800"`

이 기능은 자동으로 회전됩니다.

* 확장 ()에 타임스탬프가 있는 Dispatcher 로그 파일`logs/dispatcher.log%Y%m%d`).
...
>[!NOTE]
>
>설치 후 기본 로그 수준이 높으므로(즉, 수준 3 = 디버그) Dispatcher가 모든 오류 및 경고를 기록합니다. 이 수준은 초기 단계에서 유용합니다.
>
>그러나 이러한 수준에는 추가 리소스가 필요합니다. Dispatcher가 원활하게 작동하는 경우 *요구 사항에 따라*&#x200B;로그 수준을 낮출 수 있습니다.

### 추적 로깅 {#trace-logging}

Dispatcher에 대한 다른 개선 사항 중 버전 4.2.0은 추적 로깅도 도입했습니다.
...
1. 캐시를 올바르게 플러시하는지 확인하려면 페이지를 활성화합니다.
1. 모든 것이 올바르게 작동하면 `loglevel` to `0`.
...
헤더를 추가할 때 `X-Dispatcher-Info` 요청에 대해 Dispatcher는 대상이 캐시되었는지, 캐시됨에서 반환되었는지 또는 캐시할 수 없는지에 대해 응답합니다. 응답 헤더 `X-Cache-Info`에는 이 정보가 읽을 수 있는 형식으로 포함되어 있습니다. 이러한 응답 헤더를 사용하여 Dispatcher에 의해 캐시된 응답과 관련된 문제를 디버그할 수 있습니다.

이 기능은 기본적으로 활성화되어 있지 않으므로 응답 헤더에 대해서는 `X-Cache-Info` 포함하려면 팜에 다음 항목이 포함되어야 합니다.
...
또한, `X-Dispatcher-Info` 헤더는 값이 필요하지 않지만 `curl` 테스트의 경우 다음과 같이 헤더에 전송할 값을 제공해야 합니다.

```xml
curl -v -H "X-Dispatcher-Info: true" https://localhost/content/wknd/us/en.html
```

다음은 다음과 같은 응답 헤더가 포함된 목록입니다 `X-Dispatcher-Info` 반환:

* **캐시됨**\
   대상 파일이 캐시에 저장되고 Dispatcher가 이 파일을 전달하는 것이 유효한지 확인했습니다.
* **캐싱**\
   대상 파일이 캐시에 포함되지 않으며 Dispatcher가 출력을 캐시하여 전달하는 것이 유효하다고 확인했습니다.
* **캐싱: stat 파일이 최신입니다**
대상 파일이 캐시에 포함되어 있지만 최신 stat 파일에 의해 무효화됩니다. Dispatcher가 대상 파일을 삭제하고 출력에서 다시 만들어 전달합니다.
* **캐시 불가능: 문서 루트 없음**
...
* **캐시 불가능: 임시 파일 경로가 너무 깁니다**\
   임시 파일 이름 템플릿이 시스템에서 가능한 최장 파일 이름을 초과합니다. Dispatcher는 캐시된 파일을 실제로 만들거나 덮어쓰려면 먼저 임시 파일을 만듭니다. 임시 파일 이름은 문자가 있는 대상 파일 이름입니다 `_YYYYXXXXXX` 여기에 추가됨, 여기서 `Y` 및 `X` 은 고유한 이름을 만들기 위해 대체됩니다.
* **캐시 불가능: 요청 URL에 확장명이 없습니다**\
   요청 URL에 확장명이 없거나 파일 확장명 뒤에 경로가 있습니다(예: `/test.html/a/path`).
* **캐시 불가: 요청이 GET 또는 HEAD이 아닙니다.**
HTTP 메서드는 GET 또는 HEAD이 아닙니다. Dispatcher는 출력에 캐시하지 말아야 하는 동적 데이터가 포함되어 있다고 가정합니다.
...
* **캐시 불가능: 대상이 디렉터리입니다**\
   대상이 디렉터리입니다. 이 위치는 URL과 일부 하위 URL이 모두 캐시 가능 출력을 포함하는 일부 개념 오류를 가리킬 수 있습니다. 예를 들어 `/test.html/a/file.ext` 가 먼저 오고 캐시 가능한 출력을 포함합니다. Dispatcher는 다음에 대한 후속 요청의 출력을 캐시할 수 없습니다 `/test.html`.
...
원격 서버가 
`Dispatcher: no_cache` 헤더. Dispatcher가 출력을 캐시하도록 합니다.
* **캐시 불가: 응답 컨텐츠 길이가 0입니다.**
응답의 컨텐츠 길이는 0입니다. dispatcher는 길이가 0인 파일을 만들지 않습니다.
