...
AEM 및 Dispatcher의 모든 요소는 IPv4 및 IPv6 네트워크 모두에 설치할 수 있습니다. IPV4 [및 IPV6을 참조하십시오](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/introduction/technical-requirements.html?lang=en#ipv-and-ipv).
...
예를 들어 dispatcher.any 파일이 캐시 디렉토리와 동일한 디렉토리에 있는 경우 docroot [속성에 대해 다음 값을 사용할](#specifying-the-cache-directory) 수 있습니다.

```xml
/docroot "${PWD}/cache"
```

또 다른 예로 AEM 게시 인스턴스의 호스트 이름 `PUBLISH_IP` 을 저장하는 환경 변수를 만드는 경우 [/renders](#defining-page-renderers-renders) 속성의 다음 구성을 사용할 수 있습니다.
...
>[!NOTE]
>
>렌더링 팜을 두 개 이상 사용하는 경우 목록이 상향으로 평가됩니다. 웹 사이트에 대한 [가상 호스트를](#identifying-virtual-hosts-virtualhosts) 정의할 때 특히 유용합니다.
...
따라서 기본 가상 호스트를 `virtualhosts` 파일의 최상위 팜에 있는 `dispatcher.any` 속성 맨 위에 배치해야 합니다.
...
사용자가 팜의 페이지에 액세스하기 위해 로그인해야 할 수 있도록 렌더링 팜에 액세스할 수 있는 보안 세션을 만듭니다. 로그인한 사용자는 팜의 페이지에 액세스할 수 있습니다. CUG [에서 이 기능 사용에 대한 자세한 내용은 닫힌 사용자](https://experienceleague.adobe.com/docs/experience-manager-65/administering/security/cug.html?lang=en#creating-the-user-group-to-be-used) 그룹 만들기를 참조하십시오. 또한 라이브되기 전에 Dispatcher [보안 체크리스트를](/help/using/security-checklist.md) 참조하십시오.
...
세션 정보가 인코딩되는 방법입니다. md5 알고리즘 `md5` 을 사용한 암호화 또는 16진수 인코딩에 `hex` 사용할 수 있습니다. 세션 데이터를 암호화할 경우 파일 시스템에 액세스할 수 있는 사용자는 세션 내용을 읽을 수 없습니다. The default is `md5`.
...
세션 시간이 지난 후에 초과될 때까지 남은 시간(초)입니다. 지정하지 않은 경우 `"800"` 는 사용자의 마지막 요청 후 약 13분 동안 세션이 시간 초과됩니다.
...
AEM 인스턴스에 액세스하는 연결 시간 초과(밀리초)를 지정합니다. 기본값은 Dispatcher `"0"`가 무기한 대기하도록 합니다.

**/receiveTimeout**

응답이 허용된 시간(밀리초)을 지정합니다. 기본값은 Dispatcher `"600000"`가 10분 동안 대기하도록 하는 것입니다. 을 설정하면 시간 초과가 완전히 `"0"` 제거됩니다.
...
Dispatcher가 `getaddrinfo` 함수(IPv6용)를 사용하여 렌더링의 IP 주소를 획득하는지(IPv4용) `gethostbyname` 를 사용할지 여부를 지정합니다. 0의 값은 `getaddrinfo` 사용됩니다. 사용할 `1` 원인 값 `gethostbyname` . The default value is `0`.

이 `getaddrinfo` 함수는 IP 주소 목록을 반환합니다. Dispatcher는 TCP/IP 연결을 설정할 때까지 주소 목록을 반복합니다. 따라서 렌더링 호스트 이름이 여러 IP 주소와 연결되어 있고 이 함수에 대한 응답으로 호스트가 항상 같은 순서로 IP 주소 목록을 반환하는 경우 이 `ipv4` `getaddrinfo` 속성이 중요합니다. 이 경우 Dispatcher가 연결하는 IP 주소가 임의로 `gethostbyname` 지정되도록 함수를 사용해야 합니다.

Amazon ELB(Elastic Load Balancing)는 잠재적으로 동일한 IP 주소 목록을 가진 getaddrinfo에 응답하는 서비스입니다.

**/secure**

속성 `/secure` 에 Dispatcher의 값이 `"1"` 있으면 HTTPS를 사용하여 AEM 인스턴스와 통신합니다. 자세한 내용은 SSL을 사용하도록 [디스패처 구성을 참조하십시오](dispatcher-ssl.md#configuring-dispatcher-to-use-ssl).

**/always-resolve**

Dispatcher 버전 **4.1.6**&#x200B;을 사용하면 다음과 같이 `/always-resolve` 속성을 구성할 수 있습니다.

* 이 설정을 `"1"` 설정하면 모든 요청에서 호스트 이름을 확인합니다. (디스패처는 IP 주소를 캐시하지 않습니다.) 각 요청에 대한 호스트 정보를 가져오는 데 필요한 추가 호출로 인해 약간의 성능 영향이 있을 수 있습니다.
...
**참고:** 상태 파일에 대한 [요청은](#naming-the-statfile) 항상 거부됩니다.

>[!CAUTION]
>
>Dispatcher를 사용하여 [액세스를 제한할 때](security-checklist.md) 추가적인 고려 사항은 Dispatcher 보안 체크리스트를 참조하십시오. 또한 AEM 설치에 대한 추가 보안 세부 사항은 [AEM 보안 체크리스트를](https://experienceleague.adobe.com/docs/experience-manager-65/administering/security/security-checklist.html?lang=en#security) 참조하십시오.

이 `/filter` 섹션은 HTTP 요청의 요청 라인 부분에 있는 패턴에 따라 컨텐츠에 대한 액세스를 거부하거나 허용하는 일련의 규칙으로 구성됩니다. 섹션의 허용 목록 전략을 사용해야 `/filter` 합니다.
...
문자는 `<CRLF>` 캐리지 리턴과 줄 피드를 나타냅니다. 다음은 클라이언트가 WKND 사이트의 미국-영어 페이지를 요청할 때 수신되는 요청 라인입니다.

`GET /content/wknd/us/en.html HTTP.1.1<CRLF>`

패턴은 요청 라인의 공백 문자와 문자를 고려해야 `<CRLF>` 합니다.
...
필터를 만든 후 [페이지 액세스를](#testing-dispatcher-security) 테스트하여 AEM 인스턴스가 안전한지 확인합니다.

Dispatcher 구성 파일 `/filter` 에서 `dispatcher.any` 파일의 다음 섹션을 기초로 사용할 [수 있습니다.](#dispatcher-configuration-files)

이 예는 Dispatcher와 함께 제공되며 프로덕션 환경에서 사용하기 위한 예로서 사용되는 기본 구성 파일을 기반으로 합니다. 접두사로 `#` 고친 항목은 비활성화(주석으로 처리)되므로 보안에 영향을 줄 수 있으므로 이러한 항목을 활성화(해당 행에서 제거) `#` 하려는 경우 주의해야 합니다.
...
>[!NOTE]
>
>Apache와 함께 사용할 때 Dispatcher 모듈의 DispatcherUseProcessedURL 속성에 따라 필터 URL 패턴을 디자인합니다. (Apache [Web Server - Dispatcher용 Apache 웹 서버 구성을 참조하십시오](dispatcher-install.md##apache-web-server-configure-apache-web-server-for-dispatcher).)

>[!NOTE]
>
>Dynamic Media `0030` 에 대한 필터 및 `0031` 관련 기능은 AEM 6.0 이상에 적용됩니다.
...
>[!CAUTION]
>
>게시 환경에서 [보고서를 사용하는](https://experienceleague.adobe.com/docs/experience-manager-65/administering/operations/reporting.html?lang=en#using-reports-in-a-publish-environment) 경우 Dispatcher가 외부 방문자에 대한 액세스를 거부하도록 구성해야 `/etc/reports` 합니다.
...
단일 항목은 두 가지 중 하나 `glob` 또는 일부 조합 `method``url`을 가질 수 `query`있지만 둘 다 사용할 수는 `version`없습니다. 다음 예제에서는 `a=*` 쿼리 문자열을 허용하고 노드를 확인하는 URL에 대한 다른 모든 쿼리 문자열을 `/etc` 거부합니다.
...
>[!NOTE]
>
>렌더링이 AEM의 인스턴스인 경우 별칭 URL 서비스를 활성화하려면 소프트웨어 배포에서 [VanityURLS-Components](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/granite/vanityurls-components) 패키지를 설치해야 합니다. (자세한 내용은 [소프트웨어](https://experienceleague.adobe.com/docs/experience-manager-65/administering/contentmanagement/package-manager.html?lang=en#software-distribution) 배포 참조)
...
1. AEM 또는 CQ 페이지에 대해 구성한 별칭 URL에 대해 구성이 URL을 거부하는지 [`/filter`](#configuring-access-to-content-filter) 확인합니다. 필요한 경우 URL을 거부하는 필터를 추가합니다.
...
필요한 경우, `/propagateSyndPost` 속성을 설정하여 신디케이션 요청 `"1"` 으로 배포 요청을 전달합니다. 설정된 경우 필터 섹션에서 POST 요청이 거부되지 않도록 해야 합니다.
...
상태 파일에 내용이 없습니다. 컨텐츠가 업데이트되면 Dispatcher는 타임스탬프를 업데이트합니다. 기본 상태 파일의 이름 `.stat` 이 지정되고 doclut에 저장됩니다. 디스패처는 상태 파일에 대한 액세스를 차단합니다.

>[!NOTE]
>
>이 구성 `/statfileslevel` 이 구성되면 Dispatcher는 `/statfile` 속성을 무시하고 이름으로 `.stat` 사용합니다.
...
이 `/serveStaleOnError` 로 설정된 경우, 렌더링 서버 `"1"`가 성공적인 응답을 반환하지 않는 한 Dispatcher는 무효화된 컨텐츠를 캐시에서 삭제하지 않습니다. AEM의 5xx 응답이나 연결 시간 초과로 인해 Dispatcher가 오래된 컨텐츠를 제공하고 HTTP 상태(재유효성 검사 실패)를 사용하여 응답합니다.
...
그러나 요구 사항에 인증된 문서의 캐싱이 허용되면 다음 중 하나 `/allowAuthorized` 로 설정합니다.
...
이 `/rules` 속성은 문서 경로에 따라 캐시되는 문서를 제어합니다. 속성 `/rules` 에 관계없이 Dispatcher는 다음 상황에서 문서를 캐시하지 않습니다.
...
>[!NOTE]
>
>GET 또는 HEAD(HTTP 헤더의 경우) 메서드는 Dispatcher에서 캐시할 수 있습니다. 응답 헤더 캐시에 대한 자세한 내용은 [HTTP 응답 헤더 캐싱 섹션을](#caching-http-response-headers) 참조하십시오.

속성의 `/rules` 각 항목에는 [`glob`](#designing-patterns-for-glob-properties) 패턴과 유형이 포함됩니다.

* 이 `glob` 패턴은 문서의 경로와 일치하는 데 사용됩니다.
* 유형은 패턴과 일치하는 문서를 캐시할지 여부를 `glob` 나타냅니다. 이 값은 허용(문서를 캐시할 수 있음) 또는 거부(문서를 항상 렌더링할 수 있음)일 수 있습니다.
...
>[!NOTE]
>
>추가 헤더를 전송하여 무효화를 방지할 수 있습니다 `CQ-Action-Scope:ResourceOnly`. 캐시의 다른 부분을 무효화하지 않고 특정 리소스를 플러시하는 데 사용할 수 있습니다. 자세한 내용은 [이 페이지](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html) 및 [수동으로 발송자 캐시 무효화를](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/page-invalidate.html?lang=en#configuring) 참조하십시오.
...
이 구성으로 인해 활성화되면 다음 활동 `/content/wknd/us/en` 이 발생합니다.

* 패턴 엔이 있는 모든 파일* 가 `/content/wknd/us` 폴더에서 제거됩니다.
* 폴더가 `/content/wknd/us/en./_jcr_content` 제거됩니다.
* 구성과 일치하는 다른 모든 파일은 `/invalidate` 즉시 삭제되지 않습니다. 이러한 파일은 다음 요청이 발생하면 삭제됩니다. 본사의 예에서 `/content/wknd.html` 는 삭제되지 않으며 요청 시 `/content/wknd.html` 삭제됩니다.
...
Adobe Analytics과 AEM의 통합은 웹 사이트의 `analytics.sitecatalyst.js` 파일에 구성 데이터를 전달합니다. Dispatcher와 함께 제공되는 예제 `dispatcher.any` 파일에는 이 파일에 대한 다음 무효화 규칙이 포함되어 있습니다.
...
이 `/invalidateHandler` 속성을 사용하면 Dispatcher가 수신한 각 무효화 요청에 대해 호출되는 스크립트를 정의할 수 있습니다.

다음 인수를 사용하여 호출됩니다.

* 핸들 - 무효화된 컨텐츠 경로
* 작업 - 복제 작업(예: 활성화, 비활성화)
* 작업 범위 - 복제 작업의 범위(헤더가 전송되지 않는 경우 비어 있음 `CQ-Action-Scope: ResourceOnly` , 자세한 내용은 AEM [에서 캐시된 페이지 무효화](page-invalidate.md) 참조)
...
이 `/allowedClients` 속성은 캐시를 플러시할 수 있는 특정 클라이언트를 정의합니다. 글로빙 패턴은 IP와 일치합니다.
...
다음 예에서는 Dispatcher가 매개 변수를 무시하므로 q 매개 변수를 포함하는 요청 URL이 캐시됩니다. `q`
...
>[!NOTE]
>
>파일 글로벌 문자는 허용되지 않습니다. 자세한 내용은 전역 속성의 [패턴 디자인을 참조하십시오](#designing-patterns-for-glob-properties).
...
기본값은 소유자가 읽고, 쓰거나, 검색할 수 있도록 하고, 그룹 및 다른 사람이 읽고 검색할 수 있도록 하는 `0755` 것입니다.
...
렌더링 선택을 위한 통계를 유지할 각 문서 유형에 대한 범주를 정의합니다. 이 `/statistics` 섹션에는 섹션이 `/categories` 포함되어 있습니다. 카테고리를 정의하려면 다음 형식의 섹션 아래에 `/categories` 줄을 추가합니다.
...
예를 들어, Dispatcher 기본 `dispatcher.any` 파일은 HTML 카테고리와 다른 카테고리를 정의합니다. HTML 카테고리는 보다 구체적이며 먼저 나타납니다.
...
속성이 `/unavailablePenalty` 없으면 값이 `"1"` 사용됩니다.
...
고정 연결이 활성화되면 디스패처 모듈은 `renderid` 쿠키를 설정합니다. 이 쿠키에는 `httponly` 플래그가 없으며 보안을 강화하기 위해 추가해야 합니다. 이렇게 하려면 `httpOnly` 구성 파일 노드 `/stickyConnections` `dispatcher.any` 에서 속성을 설정합니다. 속성 값(또는 `0` )은 `1`쿠키에 속성이 추가되었는지 `renderid` `HttpOnly` 여부를 정의합니다. 기본값은 `0`입니다. 즉, 속성이 추가되지 않습니다.

플래그에 대한 자세한 내용은 `httponly` 이 페이지 [](https://www.owasp.org/index.php/HttpOnly)를 참조하십시오.

### 보안 {#secure}

고정 연결이 활성화되면 디스패처 모듈은 `renderid` 쿠키를 설정합니다. 이 쿠키에는 `secure` 플래그가 없으며 보안을 강화하기 위해 추가해야 합니다. 이렇게 하려면 `secure` 구성 파일 노드 `/stickyConnections` `dispatcher.any` 에서 속성을 설정합니다. 속성 값(또는 `0` )은 `1`쿠키에 속성이 추가되었는지 `renderid` `secure` 여부를 정의합니다. 기본값은 `0`입니다. 즉, 들어오는 요청이 안전한 **경우** 속성이 추가됩니다. 이 값이 로 설정된 경우 수신 요청이 `1`안전한지 여부에 관계없이 보안 플래그가 추가됩니다.
...
이 `/retryDelay` 속성은 Dispatcher가 팜 렌더와의 연결 라운드 시도 간에 대기하는 시간(초)을 설정합니다. 각 라운드에 대해 Dispatcher가 렌더로의 연결을 시도하는 최대 수는 팜의 렌더링 수입니다.
...
값이 명시적으로 정의되지 않은 경우 기본값은 입니다 `5`.
...
인스턴스에 이러한 구성이 `/ignoreEINTR` 있고 로그에 다음 메시지가 포함된 경우 매개 변수를 사용하십시오.
...
Dispatcher `/ignoreEINTR` 가 전체 응답을 읽을 때까지 데이터를 계속 읽도록 하는 설정 `"1"` . 기본값은 `0` 이며 옵션을 비활성화합니다.

## 전역 속성을 위한 패턴 디자인 {#designing-patterns-for-glob-properties}

Dispatcher 구성 파일의 여러 섹션에서 속성은 클라이언트 요청에 대한 선택 `glob` 기준으로 사용됩니다. 속성 값은 Dispatcher가 요청한 리소스의 경로 또는 클라이언트의 IP 주소와 같은 요청의 측면과 비교하는 패턴입니다. `glob` 예를 들어, 섹션의 항목은 `/filter` `glob` 패턴을 사용하여 Dispatcher가 작업하거나 거부한 페이지의 경로를 식별합니다.

이 `glob` 값에는 패턴을 정의하는 와일드카드 문자와 영숫자 문자가 포함될 수 있습니다.
