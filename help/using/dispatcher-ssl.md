...
>[!NOTE]
>
>SSL 인증서와 관련된 작업은 타사 제품에 바인딩됩니다. Adobe Platinum 유지 관리 및 지원 계약에 포함되지 않습니다.
...
Dispatcher를 구성하기 전에 SSL을 사용하도록 AEM 또는 CQ를 구성합니다.
...
* 이전 AEM 버전:[이 페이지](https://helpx.adobe.com/kr/experience-manager/aem-previous-versions.html)를 참조하십시오.
...
`mod_ssl`을 사용하는 Apache-2.4를 통한 요청에는 다음 예와 유사한 헤더가 포함되어 있습니다.
...
* HTTPS 요청을 처리하는 가상 호스트.
* 가상 호스트의 `renders` 섹션에는 HTTPS를 사용하는 CQ 또는 AEM 인스턴스의 호스트 이름과 포트를 식별하는 항목이 포함되어 있습니다.
* `renders` 항목에는 `1` 값의 `secure` 속성이 포함되어 있습니다.

참고:필요한 경우 HTTP 요청을 처리할 다른 가상 호스트를 만듭니다.

다음 예제 dispatcher.any 파일은 호스트 `localhost` 및 포트 `8443`에서 실행 중인 CQ 인스턴스에 HTTPS를 사용하여 연결하는 속성 값을 보여줍니다.
...
* Dispatcher는 렌더링 인스턴스 인증서의 CA를 신뢰할 수 있는지 확인합니다.
* (선택 사항) Dispatcher는 렌더링 인스턴스의 인증서가 렌더링 인스턴스의 서버 주소와 일치하는지 확인합니다.

상호 SSL을 구성하려면 신뢰할 수 있는 CA(인증 기관)에서 서명한 인증서가 필요합니다. 자체 서명된 인증서는 적절하지 않습니다. CA 역할을 하거나 타사 CA의 서비스를 사용하여 인증서에 서명할 수 있습니다. 상호 SSL을 구성하려면 다음 항목이 필요합니다.

* 렌더링 인스턴스 및 Dispatcher에 대해 서명된 인증서
* CA 인증서(CA 역할을 하는 경우)
* CA, 인증서 및 인증서 요청을 생성하기 위한 OpenSSL 라이브러리.

상호 SSL을 구성하려면 다음 단계를 수행합니다.

1. [](dispatcher-install.md) 플랫폼에 최신 버전의 Dispatcher를 설치합니다. SSL을 지원하는 Dispatcher 바이너리를 사용합니다(예: dispatcher-apache2.4-linux-x86-64-ssl10-4.1.7.tar).
1. [Dispatcher 및 ](dispatcher-ssl.md#main-pars-title-3) 렌더링 인스턴스에 대한 CA 서명 인증서를 만들거나 얻습니다.
1. [렌더링 인증서가 ](dispatcher-ssl.md#main-pars-title-6) 포함된 키 저장소를 만들고 이 인증서를 사용하도록 렌더링의 HTTP 서비스를 구성합니다.
1. [상호 SSL을 위한 Dispatcher 웹 서버 ](dispatcher-ssl.md#main-pars-title-4) 모듈을 구성합니다.

### CA 서명 인증서 만들기 또는 가져오기 {#creating-or-obtaining-ca-signed-certificates}

게시 인스턴스 및 Dispatcher를 인증하는 CA 서명 인증서를 만들거나 얻습니다.
...
1. 터미널을 열고 현재 디렉터리를 `/usr/local/ssl/misc` 등의 CA.sh 파일을 포함하는 디렉토리로 변경합니다.
1. CA를 만들려면 다음 명령을 입력한 다음 문제가 발생할 때 값을 제공합니다.
...
OpenSSL을 사용하여 타사 CA로 보내거나 CA로 서명하기 위한 인증서 요청을 만듭니다.

인증서를 만들 때 OpenSSL은 일반 이름 속성을 사용하여 인증서 보유자를 식별합니다. 렌더링 인스턴스의 인증서에 대해 Dispatcher가 게시 인스턴스의 호스트 이름과 일치하는 경우에만 인증서를 허용하도록 구성하는 경우 인스턴스 컴퓨터의 호스트 이름을 일반 이름으로 사용하십시오. ( [DispatcherCheckPeerCN](dispatcher-ssl.md#main-pars-title-11) 속성을 참조하십시오.)

1. 터미널을 열고 현재 디렉터리를 OpenSSL 라이브러리의 CH.sh 파일이 포함된 디렉토리로 변경합니다.
1. 다음 명령을 입력하고 메시지가 표시되면 값을 제공합니다. 필요한 경우 게시 인스턴스의 호스트 이름을 일반 이름으로 사용합니다. 호스트 이름은 렌더링의 IP 주소에 대한 DNS 확인 가능한 이름입니다.

   ```shell
   ./CA.sh -newreq
   ```

   타사 CA를 사용하는 경우 CA로 newreq.pem 파일을 보내어 서명하십시오. CA로 활동하는 경우 3단계로 진행합니다.
...
1. CA 인증서를 사용하여 인증서에 서명하려면 다음 명령을 입력합니다.
   ...
   CA 관리 파일이 포함된 디렉터리에 newcert.pem 및 newkey.pem이라는 두 파일이 만들어집니다. 렌더링 컴퓨터의 공개 인증서 및 개인 키입니다.

1. newcert.pem을 rendercert.pem으로 변경하고 newkey.pem을 renedkey.pem으로 변경합니다.
1. 2단계와 3단계를 반복하여 Dispatcher 모듈에 대한 새 인증서와 새 공개 키를 만듭니다. Dispatcher 인스턴스에만 사용되는 공통 이름을 사용해야 합니다.
1. newcert.pem을 discert.pem으로 변경하고 newkey.pem을 disclokey.pem으로 변경합니다.
...
1. 터미널 창에서 현재 디렉터리를 렌더링 인증서 및 개인 키의 위치로 변경합니다.
...
#### Render의 Truststore {#adding-the-ca-cert-to-the-render-s-truststore}에 CA 인증서를 추가하는 중
...
1. 렌더링 인스턴스를 실행하여 키 저장소를 신뢰하도록 JVM을 구성하려면 다음 시스템 속성을 사용합니다.
...
Publish Instance *섹션에서* SSL 사용 섹션의 지침에 따라 렌더링 인증서를 사용하여 SSL을 사용하도록 렌더링 인스턴스의 HTTP 서비스를 구성합니다.

* AEM 6.2:[SSL을 통해 HTTP 활성화](https://helpx.adobe.com/experience-manager/6-2/sites/deploying/using/config-ssl.html)
* AEM 6.1:[SSL을 통해 HTTP 활성화](https://docs.adobe.com/content/docs/en/aem/6-1/deploy/configuring/config-ssl.html)
* 이전 AEM 버전:[이 페이지를 참조하십시오.](https://helpx.adobe.com/experience-manager/aem-previous-versions.html)

### 디스패처 모듈에 대한 SSL 구성 {#configuring-ssl-for-the-dispatcher-module}

상호 SSL을 사용하도록 Dispatcher를 구성하려면 Dispatcher 인증서를 준비한 다음 웹 서버 모듈을 구성합니다.

### 통합 Dispatcher 인증서 만들기 {#creating-a-unified-dispatcher-certificate}

디스패처 인증서와 암호화되지 않은 개인 키를 하나의 PEM 파일에 결합합니다. 텍스트 편집기 또는 `cat` 명령을 사용하여 다음 예제와 유사한 파일을 만듭니다.

1. 터미널을 열고 현재 디렉터리를 dispatkey.pem 파일의 위치로 변경합니다.
...
1. 텍스트 편집기 또는 `cat` 명령을 사용하여 암호화되지 않은 개인 키와 인증서를 다음 예제와 유사한 단일 파일에 결합합니다.
...
[Dispatcher 모듈 구성](dispatcher-install.md#main-pars-55-35-1022)(`httpd.conf`에서)에 다음 속성을 추가합니다.
...
* `DispatcherCACertificateFile`:SSL 서버가 루트 기관에서 신뢰할 수 없는 CA를 표시하는 경우 사용되는 CA 인증서 파일의 경로입니다.
