...
seo-description: SSL 연결을 사용하여 AEM과 통신하도록 Dispatcher를 구성하는 방법에 대해 알아봅니다.
...
* AEM 6.2: [SSL을 통한 HTTP 활성화](https://helpx.adobe.com/experience-manager/6-2/sites/deploying/using/config-ssl.html)
* AEM 6.1: [SSL을 통한 HTTP 활성화](https://docs.adobe.com/content/docs/en/aem/6-1/deploy/configuring/config-ssl.html)
...
Dispatcher와 렌더링 컴퓨터(일반적으로 AEM 또는 CQ 게시 인스턴스) 간의 연결을 구성하여 상호 SSL을 사용합니다.
...
1. [렌더링 인증서를 포함하는 키 저장소를 생성하고](dispatcher-ssl.md#main-pars-title-6) 이를 사용하도록 렌더링의 HTTP 서비스를 구성합니다.
...
인증서를 만들 때 OpenSSL은 일반 이름 속성을 사용하여 인증서 소유자를 식별합니다. 렌더링 인스턴스의 인증서에 대해 게시 인스턴스의 호스트 이름과 일치하는 경우에만 인증서를 수락하도록 Dispatcher를 구성하는 경우 인스턴스 컴퓨터의 호스트 이름을 일반 이름으로 사용합니다. ([DispatcherCheckPeerCN](dispatcher-ssl.md#main-pars-title-11) 속성을 참조하십시오.)
...
1. CA의 인증서를 사용하여 인증서에 서명하려면 다음 명령을 입력합니다.
   ...
   CA 관리 파일이 포함된 디렉터리에 newcert.pem 및 newkey.pem이라는 두 개의 파일이 생성됩니다. 각각 렌더링 컴퓨터의 공개 인증서와 개인 키입니다.

1. newcert.pem의 이름을 rendercert.pem으로 바꾸고 newkey.pem의 이름을 renderkey.pem으로 바꿉니다.
1. 2단계와 3단계를 반복하여 Dispatcher 모듈을 위한 새 인증서와 새 공개 키를 만듭니다. Dispatcher 인스턴스에 고유한 일반 이름을 사용하는지 확인하십시오.
1. newcert.pem의 이름을 dispcert.pem으로 바꾸고 newkey.pem의 이름을 dispkey.pem으로 바꿉니다.

### 렌더링 컴퓨터에서 SSL 구성 {#configuring-ssl-on-the-render-computer}

rendercert.pem 및 renderkey.pem 파일을 사용하여 렌더링 인스턴스에서 SSL을 구성합니다.

#### JKS 형식으로 렌더링 인증서 변환 {#converting-the-render-certificate-to-jks-format}

다음 명령을 사용하여 PEM 파일인 렌더링 인증서를 PKCS#12 파일로 변환합니다. 또한 렌더링 인증서에 서명한 CA의 인증서를 포함합니다.

1. 터미널 창에서 현재 디렉터리를 렌더링 인증서 및 개인 키 위치로 변경합니다.
1. 다음 명령을 입력하여 PEM 파일인 렌더링 인증서를 PKCS#12 파일로 변환합니다. 또한 렌더링 인증서에 서명한 CA의 인증서를 포함합니다.
...
1. Java 키 저장소는 기본 별칭을 사용하여 작성됩니다. 원하는 경우 별칭을 변경합니다.
...
*게시 인스턴스에서 SSL 활성화* 섹션의 지침에 따라 렌더링 인증서를 사용하여 SSL을 사용하도록 렌더링 인스턴스의 HTTP 서비스를 구성합니다.
